...
SAMPLE_QUESTIONS_DATA = [
    ...
    {
        "question": "RESTful APIとは何ですか？",
        "answer": "RESTful APIは、HTTPプロトコルを使用してクライアントとサーバー間で通信を行うWebサービスの設計原則です。",
        "correct_answer": "RESTful APIは、HTTP/HTTPSプロトコルを使用して、リソースの作成(POST)、読み取り(GET)、更新(PUT/PATCH)、削除(DELETE)を行うWebサービスのアーキテクチャスタイルです。ステートレス性、キャッシュ可能性、統一インターフェースなどの特徴を持ち、スケーラブルで疎結合なシステムを実現します。",
        "feedback": "部分的に正確: 基本的な説明はありますが、RESTの具体的な特徴や利点の説明が不足しています",
        "is_correct": 0.5,
        "response_time": 1.7,
    },
    {
        "question": "クラウドコンピューティングの主要なサービスモデルを説明してください",
        "answer": "クラウドコンピューティングの主要なサービスモデルには、IaaS、PaaS、SaaSがあります。",
        "correct_answer": "クラウドコンピューティングの主要なサービスモデルには、Infrastructure as a Service (IaaS)、Platform as a Service (PaaS)、Software as a Service (SaaS)があります。IaaSは仮想マシンやストレージなどのインフラを提供し、PaaSは開発・実行環境を提供し、SaaSはソフトウェアをインターネット経由で提供します。各モデルは異なるレベルの抽象化と管理責任を提供します。",
        "feedback": "部分的に正確: サービスモデルの名称は挙げていますが、各モデルの詳細な説明が不足しています",
        "is_correct": 0.5,
        "response_time": 2.0,
    },
    {
        "question": "Gitのマージとリベースの違いは何ですか？",
        "answer": "マージは2つのブランチを結合し、リベースは1つのブランチのコミット履歴を別のブランチの先端に移動します。",
        "correct_answer": "Gitのマージとリベースは異なるブランチ統合方法です。マージは2つのブランチの履歴を保持したまま新しいマージコミットを作成し、履歴が分岐したままになります。一方、リベースは一方のブランチのコミットを他方のブランチの先端に順次適用し直すため、直線的な履歴になります。リベースはクリーンな履歴を作成できますが、共有ブランチでの使用は注意が必要です。",
        "feedback": "正確: マージとリベースの基本的な違いと特徴を適切に説明しています",
        "is_correct": 1.0,
        "response_time": 1.6,
    },
    {
        "question": "TensorFlowとPyTorchの主な違いは何ですか？",
        "answer": "TensorFlowとPyTorchは両方とも深層学習フレームワークですが、APIの設計思想や動的/静的グラフの扱いが異なります。",
        "correct_answer": "TensorFlowとPyTorchはディープラーニングフレームワークとして、異なるアプローチを取ります。PyTorchは動的計算グラフを採用し、Pythonのネイティブな制御フローをそのまま使用できる直感的なAPIを提供します。一方、TensorFlowは静的グラフ（現在はEager実行も可能）を使用し、本番環境への展開や大規模分散トレーニングに強みがあります。PyTorchは研究者に人気があり、TensorFlowは企業での利用が多い傾向があります。",
        "feedback": "部分的に正確: 基本的な違いは説明していますが、各フレームワークの具体的な利点や用途の説明が不足しています",
        "is_correct": 0.5,
        "response_time": 1.9,
    },
    {
        "question": "マイクロサービスアーキテクチャの利点と課題は何ですか？",
        "answer": "マイクロサービスは、アプリケーションを小さな独立したサービスに分割する設計手法で、スケーラビリティとメンテナンス性が向上します。",
        "correct_answer": "マイクロサービスアーキテクチャの利点には、サービスごとの独立した開発・デプロイ・スケーリング、技術スタックの柔軟な選択、障害の局所化などがあります。一方、課題としては、サービス間通信の複雑さ、分散トランザクション管理、監視・デバッグの難しさ、運用の複雑化などがあります。適切な設計と運用体制が必要で、小規模なプロジェクトではオーバーヘッドが大きくなる可能性があります。",
        "feedback": "部分的に正確: 基本的な説明はありますが、具体的な課題や考慮点の説明が不足しています",
        "is_correct": 0.5,
        "response_time": 1.8,
    },
]
